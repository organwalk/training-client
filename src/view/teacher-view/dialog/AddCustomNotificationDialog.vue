...
const planList = ref([])
watchEffect(() => {
  if (props.dialog && props.planList && props.planList.length !== 0) {
    planList.value = props.planList.map(item => ({
...
const loadingPeople = async (id) => {
  const res = await getStudentListByPlanId(id)
  if (res.code === 2002) {
  ...
  >
    ...
    <el-card shadow="never" style="border-radius: 10px;margin-top: -10px">
      <el-form label-position="top">
        <el-form-item label="选择培训计划">
          <el-select v-model="planId" placeholder="请选择一个培训计划" style="width: 100%" filterable>
            <el-option
